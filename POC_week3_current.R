...
server = function(input,output,session){
    ...
    if (is.null(input$file1) && is.null(input$file2)) {
      ...
      bslib::card(
        ...
        bslib::card_body(
          ...
          tags$h4("Welcome!"),
          tags$p("To begin, please upload CSV or Excel files."),
          
          tags$div(
            id = "upload_drop_card",
            class = "dropzone_big",
            fileInput("file2", label = NULL, multiple = TRUE, accept = c(".csv",".xlsx",".xls")),
            tags$div(class="hint", "Drag & drop files anywhere in this box, or click to browse")
          )
  ...
  dfCountryMaster = reactive({
    data_files = input$file1 %||% input$file2
    req(data_files)
...
ui = page_navbar(
  ...
  tags$head(
    ...
    .dropzone_big{
      ...
      padding:150px; background:#fafbfc; cursor:pointer;
    }
